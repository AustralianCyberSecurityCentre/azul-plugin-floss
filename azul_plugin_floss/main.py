...
class AzulPluginFloss(BinaryPlugin):
    ...
    FEATURES = [
        Feature(name="floss_strings", desc="Strings extracted from binary", type=str),
        Feature(name="floss_functions", desc="Functions strings are extracted from", type=str),
    ]
    ...
    def _addFlossFeature(
        ...
        flossStrings.append(AzulPluginFloss._FlossString(string, function, offset))
        if function in flossFunctions:
            flossFunctions[function] += 1
        else:
            flossFunctions[function] = 1

    def _parseJSON(stdout: str) -> tuple[list[_FlossString], dict[int, int]]:
        """Parse floss json output into azul features."""
        j = json.loads(stdout)

        flossStrings: list[AzulPluginFloss._FlossString] = []
        flossFunctions: dict[int, int] = {}

        startAddress = j["metadata"]["imagebase"]

        for stringData in j["strings"]["stack_strings"]:
            AzulPluginFloss._addFlossFeature(
                flossStrings,
                flossFunctions,
                stringData["string"],
    ...
    def _run_floss(fileDir: str, timeoutSeconds: int) -> str:
        """Run floss executable with flags set to output as json."""
        # run floss on file in directory
        cmdLineString = ["floss", "-j", "--no", "static", "--"] + [fileDir]
        ...
        with tempfile.TemporaryDirectory() as dir_name:
            ...
            if not stdout:
                ...
                raise AzulPluginFloss.FlossEmptyOutputException(error_text)
        return stdout

    def _dataToFeatures(strings: list[_FlossString], functions: dict[int, int]) -> dict[str, list[FV]]:
    ...
    def execute(self, job: Job) -> State:
        ...
        stdout = AzulPluginFloss._run_floss(
            path,
            self.cfg.run_timeout - self.cfg.run_timeout_difference,
        )
        if isinstance(stdout, State):
            return stdout
        flossStrings, flossFunctions = AzulPluginFloss._parseJSON(stdout)
        features = AzulPluginFloss._dataToFeatures(flossStrings, flossFunctions)
